...
export const subscribeToGroupEvents = () => {
  ...
  // conversation:created - fetch conversations from API instead of adding locally
  socket.on(WsEvents.ConversationCreated, (payload: ConversationCreatedPayload, eventId?: string) => {
    ...
    if (isMember) {
      // Re-fetch conversation list from API to get complete data (like Frontend_web)
      setTimeout(() => {
        chatsStore.initializeChats().then(() => {
          // Also fetch conversation detail for the new conversation
          conversationDetailStore.fetchConversationDetail(payload.conversation_id, true);
        }).catch(err => {
          console.error('[GroupEvents] Failed to fetch conversations after created:', err);
        });
      }, 0);
    }
  ...
  socket.on(WsEvents.ConversationMemberAdded, (payload: ConversationMemberAddedPayload, eventId?: string) => {
    ...
    if (addedUserIds.includes(currentUserId || '')) {
      // Current user was added to conversation - re-fetch from API (like Frontend_web)
      setTimeout(() => {
        chatsStore.initializeChats().then(() => {
          // Also fetch conversation detail for the added conversation
          conversationDetailStore.fetchConversationDetail(payload.conversation_id, true);
        }).catch(err => {
          console.error('[GroupEvents] Failed to fetch conversations after member added:', err);
        });
      }, 0);
    } else {
